...
use kind_parameters
    ...
    integer, parameter          :: s=selected_real_kind(4)
    ...
    !  interface VTK_open_file
    !    module procedure VTK_open_file, VTK_open_file_with_time
    !  end interface
...
use kind_parameters
        ...
        ! @roy23: Adding time info
        !GRID_header="FIELD FieldData 1"
        !write(unit=fd%unit) trim(GRID_header)
        !GRID_header="TIME 1 1 double"
        !write(unit=fd%unit) trim(GRID_header)
        !write (unit=fd%unit) time
        !GRID_header="CYCLE 1 1 int"
        !write(unit=fd%unit) trim(GRID_header)
        !write (unit=fd%unit) cycle
